...
/mob/verb/Create_Scroll()
	...
	//Is background a DMI?
	if(!(isfile(background_dmi) && (copytext_char("[background_dmi]",-4) == ".dmi")))
	...
	//Is foreground a DMI?
	if(!(isfile(foreground_dmi) && (copytext_char("[foreground_dmi]",-4) == ".dmi")))
	...
	//Is mask a DMI?
	if(!(isfile(mask_dmi) && (copytext_char("[mask_dmi]",-4) == ".dmi")))
